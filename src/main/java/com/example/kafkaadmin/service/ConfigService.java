...
class ConfigService extends AbstractKafkaAdminClientService {
    ...
    Map<String, List<Entry<String, String>>> getConfigs(Collection<ConfigResource> resources)
    ...
    boolean update(ConfigResource resource, Map<String, String> config)
            throws ExecutionException, InterruptedException {
        List<AlterConfigOp> alterConfigOps = config.entrySet().stream()
                .map(it -> new ConfigEntry(it.getKey(), it.getValue()))
                .map(it -> new AlterConfigOp(it, AlterConfigOp.OpType.SET)).toList();
